...
contract PanaCoin is ERC20 {
    ...
    modifier onlyOwner() {
        require(_owner == _msgSender(), "PanaCoin: caller is not the owner");
        _;
    }
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) public {
        ...
        require(signatory != address(0), "PanaCoin::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "PanaCoin::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "PanaCoin::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber) public view returns (uint256) {
        require(blockNumber < block.number, "PanaCoin::getPriorVotes: not yet determined");
    ...
    function _moveDelegates(address srcRep, address dstRep, uint256 amount) internal {
        if (srcRep != dstRep && amount > 0) {
            if (srcRep != address(0)) {
                ...
                uint256 srcRepNew = _subtract(srcRepOld, amount);
            ...
            if (dstRep != address(0)) {
                ...
                uint256 dstRepNew = _add(dstRepOld, amount);
    ...
    function _writeCheckpoint(address delegatee, uint32 nCheckpoints, uint256 oldVotes, uint256 newVotes) internal {
      uint32 blockNumber = safe32(block.number, "PanaCoin::_writeCheckpoint: block number exceeds 32 bits");
